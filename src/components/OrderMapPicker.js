...
const OrderMapPicker = ({ title = translate('components.interface.MapPicker.Select Navigator'), wrapperStyle, order, buttonStyle }) => {
    ...
    return (
        <View style={[wrapperStyle]}>
            <TouchableOpacity style={tailwind('flex flex-row items-center px-4 pb-2 mt-1')} onPress={() => actionSheetRef.current?.setModalVisible()}>
                <View style={tailwind('btn bg-cyan py-0 pl-4 pr-2')}>
                    <View style={tailwind('flex flex-row justify-start')}>
                        <View style={tailwind('border-r border-white py-2 pr-4 flex flex-row items-center')}>
                            <FontAwesomeIcon icon={faLocationArrow} style={tailwind('text-blue-50 mr-2')} />
                            <Text style={tailwind('font-semibold text-blue-50 text-base')}>{translate('Order.OrderScreen.Navigate')}</Text>
                        </View>
            ...
            <ActionSheet
                ...
                ref={actionSheetRef}
                containerStyle={tailwind('bg-dark-blue')}
